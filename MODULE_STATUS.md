...
| 生成队列 | 部分完成 | 旧 monolith 已有内存队列、并发 worker、job id、状态/进度、取消、入队限流、stale/timeout；新 SaaS 骨架已新增 `api-server/`、`worker/`、`shared/`，`POST /generate` 写 Redis 队列，`GET /status/<task_id>` 查 Redis 状态，独立 Worker 消费并最多 retry 2 次，单次执行默认 60s timeout，启动前会恢复 stale running 任务 | 把旧 `/api/generation-jobs`、样图、背景图和正式出图全部迁到 Redis Worker；接入真实生产 Redis、队列监控接口/告警、跨进程定时 sweep |
