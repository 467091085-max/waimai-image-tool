...
- 导出：支持 JPG、PNG，支持勾选、全选、单品、套餐导出，并按平台上限压缩文件大小。
...
当前制作规则：美团按 4:3 输出 800 x 600；淘宝外卖/饿了么、京东外卖/京东秒送按 1:1 输出 800 x 800。全部导出为 RGB 模式，避免客户下载后还要二次转换。若平台规则变化，只需要改 `app.py` 里的 `PLATFORMS` 常量。
