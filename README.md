...
- 风格预览：展示 5 套图库风格，选择风格后展示免费样图。
- 积分计费：基础出图 10 积分/张，品牌水印 50 积分/单，额外平台 100 积分/个平台。
- 品牌水印：支持文字水印和透明 PNG Logo，支持角标和平铺。
- 正式图预览：按单品图片、套餐图片、其他图片分组显示。
- 重做额度：每单提供免费换版额度，用完后按 10 积分/张。
- 导出：支持 JPG、PNG、WebP，支持勾选、全选、单品、套餐导出。
...
上线前建议再用对应商家后台的最新规则校验一次。如果平台规则变化，只需要改 `app.py` 里的 `PLATFORMS` 常量。
